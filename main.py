import pygame
from enemy_final import EnemyGroup
import os
...
class Game:
    ...
    def collide_base(self, enemy):  # 當 enemy 回到主堡時，return true
        """
        Return True if the enemy collide with base.
        :param enemy: object (Enemy())
        :return: Bool
        """
        x, y = self.base.center  # 主堡的座標
        width, height = self.base.w, self.base.h
        if x - width // 2 < enemy.x < x + width // 2 and y - height // 2 < enemy.y < y + height // 2: # enemy 回到主堡
            return True
        return False
    ...
    def game_run(self):
        ...
        while run:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:  # 鍵盤有輸入東西
                    if event.key == pygame.K_n and self.enemies.is_empty():  # 輸入 n & enemy 是空的
                        self.enemies.generate(3)  # 產生預備出場的 enemy

            # enemy loop
            self.enemies.campaign()
            for en in self.enemies.get():
                en.move()  # enemy 一步步移動至主堡
                # delete the object when it reach the base
                if self.collide_base(en):  # 當 enemy 抵達主堡
                    self.enemies.retreat(en)  # 把抵達主堡的 enemy 移除，並切換 PATH mode
